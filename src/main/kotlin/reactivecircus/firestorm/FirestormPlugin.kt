...
class FirestormPlugin : Plugin<Project> {
    ...
    private fun registerTasks(
        ...
        testVariants.all { testVariant ->
            ...
            val appApkProvider = (testVariant.testedVariant as? ApplicationVariant)?.packageApplicationProvider
                ?.flatMap { it.toApkFileProvider() } ?: generateDummyApk!!.flatMap { it.dummyApk }
    ...
    private fun registerAnalyzeGitChangesTask(
        ...
        return project.tasks.register(
            ...
            it.result.set(
                project.layout.buildDirectory.file("$FIRESTORM_TASK_OUTPUT_DIR/$GIT_CHANGES_ANALYSIS_RESULT_FILE")
            )
        }
    }
    ...
    private fun registerGenerateDummyApkTask(project: Project): TaskProvider<GenerateDummyApk> {
        ...
        return if (project.rootProject.hasConfiguredTask<GenerateDummyApk>()) {
            ...
            project.rootProject.tasks.register(taskName, GenerateDummyApk::class.java) {
                ...
                it.dummyApk.set(
                    project.rootProject.layout.buildDirectory.file(
                        "$FIRESTORM_TASK_OUTPUT_DIR/$DUMMY_APK_FILE"
                    )
                )
            }
        }
...
@Suppress("UnstableApiUsage")
private fun PackageAndroidArtifact.toApkFileProvider(): Provider<RegularFile> {
    return variantOutputs.flatMap { variantOutputs ->
        val output = variantOutputs.single {
            it.outputType == VariantOutputConfiguration.OutputType.SINGLE
        }
        outputDirectory.file(output.outputFileName.get())
    }
}
